...
export class ChatController {
  ...
  constructor() {
    ...
    this.mcpClient = new MultiServerMCPClient({
      ...
      prefixToolNameWithServerName: false, // Prefix tool names with server names
      additionalToolNamePrefix: "", // Add 'mcp' prefix to all tools

      // Server configuration - using SSE transport since we have HTTP servers
      mcpServers: {
        time: {
          transport: "sse",
          url: "http://time:3000/sse", // コンテナ名:内部ポート（Docker内部通信）
          reconnect: {
            enabled: true,
        ...
        "web-search": {
          transport: "sse",
          url: "http://web-search:3000/sse", // コンテナ名:内部ポート（Docker内部通信）
          reconnect: {
            enabled: true,
            maxAttempts: 3,
            delayMs: 2000,
        ...
        calculator: {
          transport: "sse",
          url: "http://calculator:3000/sse", // コンテナ名:内部ポート（Docker内部通信）
          reconnect: {
            enabled: true,
            maxAttempts: 2,
            delayMs: 2000,
        ...
        playwright: {
          transport: "sse",
          url: "http://playwright:3000/sse", // コンテナ名:内部ポート（Docker内部通信）
          reconnect: {
            enabled: true,
            maxAttempts: 3,
            delayMs: 2000,
